...
// ============================================================
import { fmt, r, esc, avgAsset, rankTotal } from './firebase.js';
...
export function buildRanking(S) {
  ...
  if (!rows.length) return html + '<div class="card"><div class="card-body">データがありません</div></div>';

  const myUid = auth.currentUser?.uid;

  // 特性マップ
  const traitMap = {
    worker:   { label: '労働者: チケット獲得間隔 2/3', icon: '👷', color: '#27ae60' },
    balancer: { label: '投資家: レアチケット確率 20%', icon: '📈', color: '#2980b9' },
    normal:   { label: '一般市民', icon: '👤', color: '#7f8c8d' }
  };
  ...
  rows.forEach((row, i) => {
    ...
    const d       = row.d;
    const bonus   = !isMe ? calcFirstBonus(row.rt) : 0; // 1位以外に補正ボーナス
    // playersMeta から特性を取得
    ...
    html += `<div class=\"rank-row${isMe?' me':''}\">
      ...
      <div style=\"flex:1;min-width:0\">
        ...
        <div class=\"hint\" style=\"white-space:nowrap;overflow:hidden;text-overflow:ellipsis\">
          ...
          株:<span class=\"din\">${fmt(d.inv||0)}</span>
        </div>
      </div>
      <div class=\"rank-score din\">${fmt(row.rt)}</div>
    </div>`;
  });
