// ============================================================
//  js/deposit.js  預金・定期預金操作
// ============================================================
import { callApi, toast, fmt, r, calcInterest } from './firebase.js';
import { S, withSubmit } from './ui.js';

export async function doDeposit() {
  await withSubmit(async () => {
    const amt = parseInt(document.getElementById('dep-in')?.value) || 0;
    if (amt <= 0) { toast('金額を入力してください'); return; }
    await callApi('deposit.php', { action: 'deposit', amount: amt });
...
export async function addDeposit() {
  await withSubmit(async () => {
    const amt = parseInt(document.getElementById('dep-add')?.value) || 0;
    if (amt <= 0) { toast('金額を入力してください'); return; }
    await callApi('deposit.php', { action: 'add', amount: amt });
...
export async function doTermDeposit() {
  await withSubmit(async () => {
    const amt  = parseInt(document.getElementById('tdep-in')?.value) || 0;
