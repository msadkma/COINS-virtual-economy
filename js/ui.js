...
// ============================================================
//  預金画面（バグ修正版：追加入金時の計算を修正）
// ============================================================
function buildDeposit(p) {
  const {depBal, tdepBal} = calcInterest(p);
  const tdl = p.termDeposit
    ? Math.max(0, p.termDeposit.days - (Date.now()-p.termDeposit.since)/86400000)
    : 0;
  return `
  <div class="card">
    <div class="card-title">普通預金
      <span class="hint" style="font-weight:400">利率 1%/日（複利）</span>
    </div>
    ${p.deposit?.principal>0 ? `
      ...
      <div class="row">
        ...
        <button class="btn btn-danger" onclick="W.withdrawDeposit()">
          全額引き出し (<span class="din">${fmt(depBal)}</span> C)
        </button>
      </div>` : `
      <div class="row">
  ...
  <div class="card">
    <div class="card-title">定期預金
      <span class="hint" style="font-weight:400">利率 2%/日（複利）・7日以上</span>
    </div>
    ${p.termDeposit?.principal>0 ? `
      <div class="row" style="margin-bottom:6px">
        <span class="hint-label">現在の残高</span><span class="spacer"></span>
      ...
      <button class="btn ${tdl>0?'btn-danger':''}"
        ...
        ${tdl>0
          ? `期限前解約（元本 <span class="din">${fmt(r(p.termDeposit.principal))}</span> C を返還）`
          : `満期引き出し (<span class="din">${fmt(tdepBal)}</span> C)`}
      </button>` : `
