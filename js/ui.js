...
exports.scheduledCompanyUpdate = onSchedule("* * * * *", async () => {
  ...
  for (const [id, c] of Object.entries(companies)) {
    ...
    if (now >= (c.nextUpdate||0)) {
      const cur        = c.price || 1;
      const totalShares = Math.max(1, c.totalShares||1); // 分母: 発行済み総株数

      // 前回更新時の予算を記録しておき、増加分のみ計上（減少は0）
      const prevBudget    = c.budgetAtLastUpdate ?? (c.totalBudget||0);
      const currentBudget = c.totalBudget||0;
      const budgetIncrease = Math.max(0, currentBudget - prevBudget);

      // 直近の売上合計（buyProductで積算・更新後リセット）
      const recentSales = c.recentSales || 0;

      // 購入中の株数 × 直近の株価（流通していない＝誰かが保有している株）
      const soldShares   = totalShares - Math.max(0, c.circulatingShares||0);
      const soldValue    = r(soldShares * cur);

      // 新計算式:
      // (直近売上 + 予算増加額 + 購入済み株数×直近株価) / 総発行株数
      const numerator  = r(recentSales + budgetIncrease + soldValue);
      const newPrice   = Math.max(1, r(numerator / totalShares));
      ...
      await dbPatch(`companies/${id}`, {
        ...
        budgetAtLastUpdate: currentBudget, // 次回比較用に現在予算を保存
        recentSales:        0,             // 売上リセット
      });
    }
...
exports.issueMoreShares = onCall(CALL_OPTS, async (request) => {
  ...
  // 新しい株価を新計算式で算出
  // 購入済み株数 = 総株数 - 流通株数（追加発行分は流通に追加されるため除外）
  const soldShares  = newTotalShares - newCirculatingShares;
  const soldValue   = r(soldShares * (c.price||1));
  const recentSales = c.recentSales || 0;
  const prevBudget  = c.budgetAtLastUpdate ?? newBudget;
  const budgetIncrease = Math.max(0, newBudget - prevBudget);
  const newPrice    = Math.max(1, r((recentSales + budgetIncrease + soldValue) / Math.max(1, newTotalShares)));
