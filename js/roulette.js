// ============================================================
//  js/roulette.js  ルーレットベット・演出・開催処理
// ============================================================
...
// ---- ホイール描画 ----
const SLOT_COUNT = 38;
function slotColor(v) { return (v===0||v===37) ? '#1d9e75' : isRed(v) ? '#c0392b' : '#2c2c2c'; }
function slotLabel(v) { return v===37 ? '00' : String(v); }

function drawWheel(ctx, rotRad) {
  const cx=130, cy=130, outerR=128, innerR=36, sa=(Math.PI*2)/SLOT_COUNT;
  ctx.clearRect(0,0,260,260);
  ctx.beginPath(); ctx.arc(cx,cy,outerR+2,0,Math.PI*2); ctx.fillStyle='#8B6914'; ctx.fill();
  for (let i=0; i<SLOT_COUNT; i++) {
    ...
    ctx.save(); ctx.translate(cx+tr*Math.cos(mid), cy+tr*Math.sin(mid));
    ctx.rotate(mid+Math.PI/2); ctx.fillStyle='#fff';
    ctx.font='bold 8px system-ui'; ctx.textAlign='center'; ctx.textBaseline='middle';
    ctx.fillText(slotLabel(v),0,0); ctx.restore();
  }
  ctx.beginPath(); ctx.arc(cx,cy,innerR,0,Math.PI*2); ctx.fillStyle='#1a1a1a'; ctx.fill();
...
export function spinWheel(targetVal) {
  return new Promise(resolve => {
    ...
    const endRot   = -(WHEEL_ORDER.indexOf(targetVal)*sa) - (3+Math.floor(Math.random()*3))*Math.PI*2;
...
export async function processRoulette() {
  try {
    ...
    if (data.had_bet) {
      ...
      if (subEl) subEl.textContent = data.win > 0
        ? `🎉 +${fmt(data.win)} COIN 獲得！`
        : '残念... また次回！';
    }
  } catch(e) {
    // "Not time yet" や "Already processing" は無視
...
export function buildRoulette(p, S) {
  ...
  const oDot=type=>otherBets[type]?'<span class="other-bet-dot"></span>':'';
  ...
  const numGrid=(()=>{
    ...
    for(let row=0;row<3;row++){
      ...
      for(let col=0;col<12;col++){
        const n=col*3+(3-row);
        cells+=`<div class="r-cell ${isRed(n)?'r-red':'r-black'} ${bets['num_'+n]?'sel':''}"
          onclick="W._toggleBet('num_${n}')">${n}${otherBets['num_'+n]?'<span class="other-bet-dot"></span>':''}</div>`;
      }
  ...
  <div class="card">
    ...
    ${rd?.last!=null?`<div class="hint" style="margin-top:6px">前回の結果: <strong class="din">${rd.last===37?'00':rd.last}</strong></div>`:''}
  </div>

  ${myBet?`<div class="card">
    <div class="row"><span style="font-weight:700">📌 自分のベット済み</span><span class="spacer"></span>
    <span class="badge badge-red din">${fmt(r(myBet.amount))} COIN</span></div>
    <div class="hint" style="margin-top:4px">${Object.entries(myBet.bets||{}).map(([k,v])=>`${k}:<span class="din">${r(v)}</span>C`).join(' | ')}</div>
  </div>`:''}

  ${Object.keys(otherBets).length>0?`<div class="card">
    <div class="card-title" style="margin-bottom:6px">👥 他プレイヤーのベット <span class="hint">(🟡 = ベットあり)</span></div>
    <div class="hint">${Object.entries(otherBets).map(([k,v])=>`${k}: <span class="din">${fmt(v)}</span>C`).join(' | ')}</div>
  </div>`:''}

  <div class="card">
    <div class="row" style="margin-bottom:4px">
      <span style="font-size:12px;color:#888">賭け可能残枠:
        <strong class="din" style="color:${remainLimit<=0?'#c0392b':'#1a1a1a'}">${fmt(remainLimit)} COIN</strong>
      </span>
    </div>
    ...
    <div style="display:grid;grid-template-columns:38px 1fr;gap:3px;margin-bottom:3px">
      <div style="display:flex;flex-direction:column;gap:2px">
        <div class="r-cell r-green ${bets['num_0']?'sel':''}" onclick="W._toggleBet('num_0')"
             style="flex:1;display:flex;align-items:center;justify-content:center;position:relative">0${oDot('num_0')}</div>
        <div class="r-cell r-green ${bets['num_37']?'sel':''}" onclick="W._toggleBet('num_37')"
             style="flex:1;display:flex;align-items:center;justify-content:center;position:relative">00${oDot('num_37')}</div>
      </div>
    ...
    <div class="row" style="margin-top:10px;gap:6px">
      <button class="btn btn-primary" onclick="W.placeBet()"
              ${betTotal<=0||remainLimit<=0?'disabled':''}>ベット確定 (<span class="din">${fmt(betTotal)}</span> COIN)</button>
      <button class="btn" onclick="W._clearBets()">クリア</button>
      <span class="hint">手持ち: <span class="din">${fmt(r(p.coins||0))}</span> C</span>
    </div>
    <div class="hint" style="margin-top:4px">🟡 = 他プレイヤーがベット中 | 数字36倍/赤黒偶奇高低2倍/列ダズン3倍</div>
  </div>`;
}
